...
/****** Object:  StoredProcedure [dbo].[fn_dba_HexToInt]    Script Date: 05/19/2010 17:43:35 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE  [dbo].[fn_dba_HexToInt] 
...
/****** Object:  StoredProcedure [dbo].[OzProductSearch]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[OzProductSearch]
@active TINYINT,
@google_published TINYINT,
@category_id NUMERIC(19,0),
@brand_id INT,
@search VARCHAR(100) = '',
@order VARCHAR(50) = 'RELEVANCEASC',
@first_result INT = 1,
@max_results INT = 1000000
AS
BEGIN

	IF(@order = 'RELEVANCEASC') BEGIN
		SELECT A.* FROM
		(
		SELECT ROW_NUMBER() OVER (ORDER BY CONTENT.TITLE ASC, CATEGORY.TITLE ASC, COUNT(DISTINCT(TAG1.ID)) + COUNT(DISTINCT(TAG2.ID)) ASC, TYPE.TITLE ASC) AS RowNum,
		CONTENT.ID 'PRODUCT_ID',
		CONTENT.TITLE 'PRODUCT', 
		TYPE.TITLE 'TYPE', 
		CATEGORY.ID 'CATEGORY_ID',
		CATEGORY.TITLE 'CATEGORY', 
		RESOURCE.TITLE 'VENDOR', 
		CONTENT.ACTIVE 'ACTIVE', 
		CONTENT.GOOGLE_PUBLISHED 'STATUS', 
		COUNT(DISTINCT(LANDING.ID)) 'LANDING_PAGE_QUANTITY', 
		COUNT(DISTINCT(TAG1.ID)) + COUNT(DISTINCT(TAG2.ID))  'TAG_QUANTITY'
		FROM CMS_CONTENT CONTENT
		LEFT OUTER JOIN CMS_MEDIA_TYPE_GROUP TYPE ON CONTENT.MEDIA_TYPE_GROUP_ID = TYPE.ID
		LEFT OUTER JOIN CMS_CONTENT_CATEGORY CONTCAT ON CONTENT.ID = CONTCAT.CONTENT_ID
		LEFT OUTER JOIN CMS_CATEGORY CATEGORY ON CONTCAT.CATEGORY_ID = CATEGORY.ID
		LEFT OUTER JOIN LPM_TAG_CONTENT TAGCONT ON CONTENT.ID = TAGCONT.CONTENT_ID
		LEFT OUTER JOIN LPM_TAG TAG1 ON TAGCONT.TAG_ID = TAG1.ID
		LEFT OUTER JOIN LPM_TAG_CATEGORY TAGCAT ON CATEGORY.ID = TAGCAT.CATEGORY_ID
		LEFT OUTER JOIN LPM_TAG TAG2 ON TAGCAT.TAG_ID = TAG2.ID
		LEFT OUTER JOIN CMS_RESOURCE RESOURCE ON CONTENT.RESOURCE_ID = RESOURCE.ID
		LEFT OUTER JOIN COMMONS_CUSTOMER CUSTOMER ON CONTENT.CUSTOMER_ID = CUSTOMER.ID
		LEFT OUTER JOIN COMMONS_BRAND BRAND ON CUSTOMER.ID = BRAND.CUSTOMER_ID
		LEFT OUTER JOIN LPM_LANDING_PAGE LANDING ON CONTENT.ID = LANDING.CONTENT_ID
		WHERE 
		CONTENT.ACTIVE = ISNULL(@active, CONTENT.ACTIVE) AND
		CONTENT.GOOGLE_PUBLISHED = ISNULL(@google_published, CONTENT.GOOGLE_PUBLISHED) AND
		CATEGORY.ID = ISNULL(@category_id, CATEGORY.ID) AND
		BRAND.ID = ISNULL(@brand_id, BRAND.ID) AND
		(
		(CONTENT.TITLE LIKE '%'+ @search +'%') OR 
		(TYPE.TITLE LIKE '%'+ @search +'%') OR 
		(CATEGORY.TITLE LIKE '%'+ @search +'%') OR 
		(TAG1.VALUE LIKE '%'+ @search +'%') OR
		(TAG2.VALUE LIKE '%'+ @search +'%') OR
		(RESOURCE.TITLE LIKE '%'+ @search +'%')
		)
		GROUP BY CONTENT.ID, CONTENT.TITLE, TYPE.TITLE, CATEGORY.ID, CATEGORY.TITLE, RESOURCE.TITLE, CONTENT.ACTIVE, CONTENT.GOOGLE_PUBLISHED
		) A
		WHERE A.ROWNUM >= @first_result
		AND A.ROWNUM < @max_results
	END
	ELSE BEGIN
		IF(@order = 'RELEVANCEDESC') BEGIN
			SELECT A.* FROM
			(
			SELECT ROW_NUMBER() OVER (ORDER BY CONTENT.TITLE DESC, CATEGORY.TITLE DESC, COUNT(DISTINCT(TAG1.ID)) + COUNT(DISTINCT(TAG2.ID)) DESC, TYPE.TITLE DESC) AS RowNum,
			CONTENT.ID 'PRODUCT_ID',
			CONTENT.TITLE 'PRODUCT', 
			TYPE.TITLE 'TYPE', 
			CATEGORY.ID 'CATEGORY_ID',
		...
		END 
		ELSE BEGIN
			IF(@order = 'CONTENTASC') BEGIN
				SELECT A.* FROM
				(
				SELECT ROW_NUMBER() OVER (ORDER BY CONTENT.TITLE ASC) AS RowNum,
				CONTENT.ID 'PRODUCT_ID',
				CONTENT.TITLE 'PRODUCT', 
				TYPE.TITLE 'TYPE', 
				CATEGORY.ID 'CATEGORY_ID',
			...
			END 
			ELSE BEGIN
				IF(@order = 'CONTENTDESC') BEGIN
					SELECT A.* FROM
					(
					SELECT ROW_NUMBER() OVER (ORDER BY CONTENT.TITLE DESC) AS RowNum,
					CONTENT.ID 'PRODUCT_ID',
					CONTENT.TITLE 'PRODUCT', 
					TYPE.TITLE 'TYPE', 
					CATEGORY.ID 'CATEGORY_ID',
				...
				END 
				ELSE BEGIN
					IF(@order = 'CATEGORYASC') BEGIN
						SELECT A.* FROM
						(
						SELECT ROW_NUMBER() OVER (ORDER BY CATEGORY.TITLE ASC) AS RowNum,
						CONTENT.ID 'PRODUCT_ID',
						CONTENT.TITLE 'PRODUCT', 
						TYPE.TITLE 'TYPE', 
						CATEGORY.ID 'CATEGORY_ID',
					...
					END 
					ELSE BEGIN
						IF(@order = 'CATEGORYDESC') BEGIN
							SELECT A.* FROM
							(
							SELECT ROW_NUMBER() OVER (ORDER BY CATEGORY.TITLE DESC) AS RowNum,
							CONTENT.ID 'PRODUCT_ID',
							CONTENT.TITLE 'PRODUCT', 
							TYPE.TITLE 'TYPE', 
							CATEGORY.ID 'CATEGORY_ID',
						...
						END 
						ELSE BEGIN
							IF(@order = 'TAGASC') BEGIN
								SELECT A.* FROM
								(
								SELECT ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT(TAG1.ID)) + COUNT(DISTINCT(TAG2.ID)) ASC) AS RowNum,
								CONTENT.ID 'PRODUCT_ID',
								CONTENT.TITLE 'PRODUCT', 
								TYPE.TITLE 'TYPE', 
								CATEGORY.ID 'CATEGORY_ID',
							...
							END 
							ELSE BEGIN
								IF(@order = 'TAGDESC') BEGIN
									SELECT A.* FROM
									(
									SELECT ROW_NUMBER() OVER (ORDER BY COUNT(DISTINCT(TAG1.ID)) + COUNT(DISTINCT(TAG2.ID)) DESC) AS RowNum,
									CONTENT.ID 'PRODUCT_ID',
									CONTENT.TITLE 'PRODUCT', 
									TYPE.TITLE 'TYPE', 
									CATEGORY.ID 'CATEGORY_ID',
								...
								END 
								ELSE BEGIN
									IF(@order = 'TYPEASC') BEGIN
										SELECT A.* FROM
										(
										SELECT ROW_NUMBER() OVER (ORDER BY TYPE.TITLE ASC) AS RowNum,
										CONTENT.ID 'PRODUCT_ID',
										CONTENT.TITLE 'PRODUCT', 
										TYPE.TITLE 'TYPE', 
										CATEGORY.ID 'CATEGORY_ID',
									...
									END 
									ELSE BEGIN
										IF(@order = 'TYPEDESC') BEGIN
											SELECT A.* FROM
											(
											SELECT ROW_NUMBER() OVER (ORDER BY TYPE.TITLE DESC) AS RowNum,
											CONTENT.ID 'PRODUCT_ID',
											CONTENT.TITLE 'PRODUCT', 
											TYPE.TITLE 'TYPE', 
											CATEGORY.ID 'CATEGORY_ID',
...
/****** Object:  StoredProcedure [dbo].[OzReports_Conversion_Combo_report_globalbilling_FINAL]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[OzReports_Conversion_Combo_report_globalbilling_FINAL]
	@startPeriod DATETIME,
	@endPeriod DATETIME,
	@state int = 4

--exec OzReports_Conversion_Combo_report_globalbilling_FINAL '20091101','20091115',4
AS
BEGIN
...
select
	x.day as 'DAY',
	x.combo as 'COMBO',
	sum(x.ambos_os_canais) as 'BOTH',
	sum(x.canal_cpa_1) as '2 Msgs CHANNEL ONLY',
	sum(x.canal_cpa_05) as '1 Msg CHANNEL ONLY'
	from (
		select 
		 convert(varchar(20),lpmt.requesttime,112) as 'Day',
		 cmsc.title as 'COMBO', 
		 case
		 --when sum(lpmtcd.current_cpa_weight) >= 2.0 then 2  and sum(lpmtcd.current_cpa_weight) < 2 
		 when sum(lpmtcd.current_cpa_weight) >= 1.5 then 1 else 0
		 end as 'ambos_os_canais'
		,
		case
		 when sum(lpmtcd.current_cpa_weight) >= 1 and sum(lpmtcd.current_cpa_weight) < 1.5 then 1 else 0
		end as 'canal_cpa_1'
		,
		case
		 when sum(lpmtcd.current_cpa_weight) <= 0.5  then 1 else 0
		end as 'canal_cpa_05'

		from  lpm_transaction lpmt
		 join cms_content cmsc on lpmt.content_id = cmsc.id --combo
		 join lpm_transaction_combo_detail lpmtcd on lpmt.id = lpmtcd.transaction_id
		 join cms_content cont on lpmtcd.content_id = cont.id --midia
		where
		 cmsc.is_combo = 1 
		and
		 lpmt.requesttime between @startPeriod and dateadd(day, +1, @endPeriod)
		and
		 lpmtcd.transaction_state_type_id = @state
		group by 
			cmsc.title ,
			convert(varchar(20),lpmt.requesttime,112), 
			lpmtcd.transaction_state_type_id,
			lpmt.user_id,
			lpmt.id
	)x
	group by
...
/****** Object:  StoredProcedure [dbo].[sp_dba_7z]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
--jms
select @cmd = '"c:\Program Files (x86)\7-Zip\7z.exe" a e:\ozonion\backup\jms\'
+convert(char(8),dateadd(dd,-1,@data),112)+'.7z e:\ozonion\backup\jms\*'+convert(char(8),dateadd(dd,-1,@data),112)+'*.*'
exec xp_cmdshell  @cmd
exec xp_cmdshell ' dir e:\ozonion\backup\jms\*.7z'
...
/****** Object:  StoredProcedure [dbo].[sp_dba_bad]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_bkp]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_cachePlan]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_counters]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_ErrorLog]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_fataBKP]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
create proc [dbo].[sp_dba_fataBKP]
as
set nocount on
...
/****** Object:  StoredProcedure [dbo].[sp_dba_fragmentation]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_indexCache]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_io]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_job]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_JobsOutput]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_kill]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
where d.name = @parm  and spid <> @@spid
...
/****** Object:  StoredProcedure [dbo].[sp_dba_lock]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_mail]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_monitor]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_monitorLS]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_osbufferdescriptors_agg]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_OverlapRole]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_pr]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_Proc]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_proc2]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_proc3]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_readerrorlog]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_SearchCachedPlans]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_startup]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_string]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_tb]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_tran]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_version]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_vw]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[sp_dba_wait]    Script Date: 05/19/2010 17:43:37 ******/
SET ANSI_NULLS ON
GO
