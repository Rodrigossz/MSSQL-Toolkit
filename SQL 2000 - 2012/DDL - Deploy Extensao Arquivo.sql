...
create table CategoriaArquivo (
id tinyint identity(1,1) primary key,
nome tddesc not null)
go

insert CategoriaArquivo select 'Documentos'
insert CategoriaArquivo select 'Musicas'
insert CategoriaArquivo select 'Fotos'
insert CategoriaArquivo select 'Videos'
insert CategoriaArquivo select 'Outros'
insert CategoriaArquivo select 'Excluidos'
...
set identity_insert.extensao on
insert extensao (id,nome,categoriaArquivoId) values (2,	'Documentos',1)
insert extensao (id,nome,categoriaArquivoId) values (3,	'Musicas',2)
insert extensao (id,nome,categoriaArquivoId) values (4,	'Fotos',3)
insert extensao (id,nome,categoriaArquivoId) values (5,	'Videos',4)
insert extensao (id,nome,categoriaArquivoId) values (6,	'Outros',5)
set identity_insert.extensao off

create table Extensao (
id smallint identity(1,1) primary key,
nome tddesc not null,
categoriaArquivoId tinyint not null references categoriaArquivo)
go
create index Extensao_ID01 on Extensao (categoriaArquivoId) include (nome)
go

--drop index Extensao.Extensao_ID02
--go

create unique index Extensao_ID02 on Extensao (nome) 
go

create table EspacoPc (
id int identity(1,1) primary key,
dataHora smalldatetime not null,
extensaoId smallint not null references Extensao,
pcId int not null references Pc,
qtd int not null,
tamanhoMB int not null)
go

create index EspacoPc_ID01 on EspacoPc (pcId) include (extensaoId)
go
create index EspacoPc_ID02 on EspacoPc (dataHora) include (extensaoId)
go
create index EspacoPc_ID03 on EspacoPc (extensaoId) include (pcId)
go
...
/****** Object:  StoredProcedure [dbo].[pr_Extensao_sel]    Script Date: 07/19/2011 18:10:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[pr_Extensao_sel_nome]	@nome tddesc WITH EXECUTE AS OWNERASSET NOCOUNT ONSELECT * FROM Extensao (nolock) WHERE nome = @nomeSET NOCOUNT OFFGO
