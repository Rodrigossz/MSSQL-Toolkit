...
go
create index Broadcast_ID01 on Broadcast (dataLimiteEnvio) include (id);
go

create table BroadcastCliente (
id int identity(1,1) primary key,
broadcastId int not null references broadcast,
clienteId int not null)
go

create index BroadcastCliente_ID01 on BroadcastCliente (clienteid) include (broadcastId)
go
create index BroadcastCliente_ID02 on BroadcastCliente (broadcastId) include (clienteid)
go

create proc pr_BroadcastCliente_sel_clienteId_Pendente
	@Clienteid int
...
SELECT b.* 
FROM Broadcast b (nolock)
WHERE dataLimiteEnvio >= GETDATE() and Not exists 
(select 1 from broadcastCliente bc (nolock) where bc.broadcastId = b.id and bc.clienteId = @Clienteid)
...
SET @GenerateProcsFor = 'Broadcast'
...
DECLARE @GenerateProcsFor varchar(100)
SET @GenerateProcsFor = 'BroadcastCliente'
--SET @GenerateProcsFor = ''
